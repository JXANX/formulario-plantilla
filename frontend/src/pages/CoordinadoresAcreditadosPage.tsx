...
export default function CoordinadoresAcreditadosPage() {
  ...
  const handleSelectPuesto = (puesto: Puesto) => {
    ...
    fetch(`${API_URL}/api/catalogo/puestos/${puesto.id}/acreditados`, {
      ...
      .catch(() => {
        toast.error('Error al cargar los Acreditados del Puesto');
        setLoadingTestigos(false);
      });
  };
  ...
  const handleOpenAssignDialog = (puesto: Puesto) => {
    setSelectedPuesto(puesto);
    setSelectedTestigoForCoord(puesto.coordinadorAcreditado || null);
    setOpenDialog(true);
  ...
  const handleExportPuestoPDF = () => {
    ...
    autoTable(doc, {
      ...
      head: [['COORDINADOR/A']],
      body: [
        [selectedPuesto.coordinadorAcreditado ? 
          `${selectedPuesto.coordinadorAcreditado.nombreCompleto}\nDocumento: ${selectedPuesto.coordinadorAcreditado.documento}   |   Celular: ${selectedPuesto.coordinadorAcreditado.celular}\nCorreo: ${selectedPuesto.coordinadorAcreditado.correo || 'N/A'}`
          : 'SIN ASIGNAR']
      ],
              ...
              <Card sx={{ borderRadius: 0, border: `1px solid ${J.border}`, boxShadow: 'none', mb: 3, bgcolor: '#fff' }}>
                <CardContent sx={{ p: 2.5 }}>
                  <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                    <Box sx={{ 
                      width: 48, height: 48, borderRadius: 0, bgcolor: selectedPuesto.coordinadorAcreditado ? 'rgba(41, 82, 204, 0.1)' : 'rgba(26, 31, 46, 0.05)', 
                      display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: `1px solid ${J.border}` 
                    }}>
                      <AssignmentIndIcon sx={{ color: selectedPuesto.coordinadorAcreditado ? J.blue : J.textMuted, fontSize: '24px' }} />
                    </Box>
                    <Box sx={{ flex: 1 }}>
                          ...
                          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, mt: 1.5, pt: 1.5, borderTop: `1px solid ${J.border}` }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              ...
                              <Typography sx={{ fontSize: '13px', color: J.ink }}>
                                {selectedPuesto.coordinadorAcreditado.celular}
                              </Typography>
                            </Box>
                            {selectedPuesto.coordinadorAcreditado.correo && (
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
